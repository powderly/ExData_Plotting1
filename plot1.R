##---------------------------------------------------------------------------------##
## plot1.R is a script that downloads recreates 1of 4 varied plots from R.Peng's   ##
...
plot1 <- function(){
        ...
        hist(as.numeric(SubsetData$Global_active_power), main="Global Active Power", xlab="Global Active Power (kilowatts)", col="red", 
             cex.lab=0.75, cex.axis=0.75, cex.main=1, cex.sub=0.75) ##reduce font size to match the figures more closely
        
        ## --------------COPY SCREEN DEVICE to PNG-------------------##
        print("Copying Plot to plot1.png")
        dev.copy(png, file = "plot1.png") ## copy from the screen device to a PNG file
        print("PNG created.")
